...
from plum._type import _ARG_KEYS, _NO_PARTS, CacheSpec, _canonical
...
class Resolver:
    ...
    def __init__(
        ...
        self.cache_spec: CacheSpec | None = _NO_PARTS
    ...
    def register(self, method: Method) -> None:
        ...
        for m in self.methods:
            ...
            if sub is None:
                ...
                return
            if sub is not acc:
                # `acc` starts canonical and stays so, which is what lets the check
                # above skip the union -- and so the allocation -- for the methods
                # that agree, which is nearly all of them.
                acc = _canonical(acc | sub)
    ...
    def resolve(
        ...
        Returns:
            :class:`.method.Method`: The method whose signature most specifically
                satisfies `target`.
        """
        if methods is None:
