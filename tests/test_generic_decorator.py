...
@pytest.mark.parametrize(
    ("slots", "bases"),
    [
        pytest.param(("__dict__",), (), id="slots-name-__dict__"),
        pytest.param((), ("inherited",), id="base-declares-the-slot"),
        pytest.param("__orig_class__", (), id="slots-is-a-string"),
    ],
)
def test_slots_warning_only_when_the_attribute_really_cannot_land(slots, bases):
    """The check asks whether an instance can carry `__orig_class__`, not whether
    this class's own `__slots__` happens to name it."""

    class Base:
        __slots__ = ("__orig_class__",)

    parents = (Base, Generic[T]) if bases else (Generic[T],)
...
def test_slots_warning_still_fires_when_it_should():
    """A slotted class with no `__dict__` and no such slot genuinely cannot."""
...
def test_a_non_none_init_return_still_raises():
    """`@generic` must not swallow the rejection of a non-`None` `__init__` return.

    Only the exception type is asserted: interpreted, CPython raises
    `__init__() should return None, not 'int'`, while a `mypyc`-compiled wrapper
    rejects it earlier with `None object expected; got int`. Both are a `TypeError`
    and both refuse the value, which is the behaviour being guarded.
    """

    @plum.generic
    class C(Generic[T]):
        def __init__(self):
            return 1  # noqa: PLE0101 -- the bug under test
